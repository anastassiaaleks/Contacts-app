...
class ContactsApp extends Contacts{
    ...
    init(dataContacts){  //отрисовка главного контейнера
    ...
    editForm(elem, that){
        ...
        modalWindowUser.innerHTML=`
            <div>
                ...
                <input type="text" class="modalAddress" placeholder='Address' value='${elem.address.suite || elem.address}'>
...
(async function getUsers(){
    if(JSON.parse(localStorage.getItem('dataContactsLocal')) && localStorage.dataContactsLocal!=='[]'){
        this.dataContacts = JSON.parse(localStorage.getItem('dataContactsLocal'))
    } else {
        let response = await fetch('https://jsonplaceholder.typicode.com/users')
        if (response.ok) this.dataContacts = await response.json()
        else console.error("Ошибка HTTP: " + response.status)
    }        
}().then(()=>{
    this.conApp=new ContactsApp()
    conApp.init(this.dataContacts)
    conApp.localUpdate()
}))
